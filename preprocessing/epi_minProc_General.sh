...
###Set up for Study Forrest but meant to be mostly generalizeable


###########!!!!!!!!!ASSUMPTIONS Made!!!!!!!!!!!!!#############
#1) Assumes T-shift is seq plus. This was set up based on Forrest
#2) 
###########!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!##########################
...
task=$3 ##What do you want the folder to be called
...
epiWC=$4 ##Wildcard matching pattern in quotes that will be passed to ls to find all EPIs that you want aligned together. This should be all runs of task. Make sure the order of WC matching is correct "sub-01_ses-movie_task-movie_run*_bold.nii.gz". The program will align all runs to the mean epi from the first run.
epiRun=$5 #nunmber 1 through total number of runs that represents the run you want to process should always originally be called as 1 and then this script will run the rest of the runs internally
threads=1 #default in case thread arg is not passed
threads=$6
numRuns=$(ls $epiWC | wc -l)
outDir=${subWD}/${sub}/${task}_run${epiRun}
tmpDir=${outDir}/tmp
...
if [[ -f ${outDir}/epiWarped.nii.gz ]];then
	echo ""
	echo "!!!!!!!!!!!!!!!!!!!!!$sub data is already processed!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
if [[ $epiRun == "1" && $epiNum -gt 1 ]];then
	3dcalc -a ${tmpDir}/epi_dt.nii.gz'[0]' -expr 'a' -prefix ${outDir}/volRegBase.nii.gz
	#####set up and submit a job to Process rest2 no that we have a volReg template from rest1
	for i in $(seq 2 $epiNum);do 
		echo "/mnt/BIAC/munin2.dhe.duke.edu/Hariri/DNS.01/Analysis/Max/scripts/Pipelines/epi_minProc_General.sh $sub ${subWD} ${task} \"${epiWC}\" ${i} ${threads} >> ${subDir}/QA/LOG.${task}${i}" >> ${outDir}/swarm.${task}.epiRuns
	done
	echo ""
	echo "#########################################################################################################"
	echo "############Submiting Jobs For $epiNum minus one other jobs for the other runs###########################"
	echo "#########################################################################################################"
	echo ""
	swarmBiac ${outDir}/swarm.${task}.epiRuns DNS.01 ${threads}
fi
voxSize=$(@GetAfniRes ${tmpDir}/epi_dtvbm.nii.gz)
...
if [[ $task != "rest1" || $task != "rest2" ]];then
	...
	3dcalc -a ${tmpDir}/epiWarped_blur6mm.nii.gz -b ${tmpDir}/mean.epiWarped_blur6mm.nii.gz -expr 'min(200, a/b*100)*step(a)*step(b)' -prefix ${outDir}/epiWarped_blur6mm.nii.gz 
fi
echo ""
echo "#########################################################################################################"
...
echo "tSNR,rawFWHM,warpedFWHM,FDavg,FDsd,DVARSavg,DVARSsd,FD25Per,FD50Per,epi2TemplateCor,epi2highResCor,highRes2TemplateCor" > ${QADir}/${task}_run${epiRun}.QCmeasures.txt
echo "$tSNR,$rawFWHM,$warpedFWHM,$FDavg,$FDsd,$DVARSavg,$DVARSsd,$FD25Per,$FD50Per,$epi2TemplateCor,$epi2highResCor,$highRes2TemplateCor" >> ${QADir}/${task}_run${epiRun}.QCmeasures.txt
...
CreateTiledMosaic -i ${tmpDir}/epi2highResBBR.nii.gz -r ${tmpDir}/epi2highResBBREdgesRBG.nii.gz -o ${QADir}/${task}_run${epiRun}.epi2HighResBBRAlignmentCheckAxial.png -a 0.8 -t -1x-1 -d 2 -p mask -s [5,mask,mask] -x ${tmpDir}/epi2highResBBREdgesRBGstep.nii.gz -f 0x1  #Create Montage taking images in axial slices every 5 slices
CreateTiledMosaic -i ${tmpDir}/epi2highResBBR.nii.gz -r ${tmpDir}/epi2highResBBREdgesRBG.nii.gz -o ${QADir}/${task}_run${epiRun}.epi2HighResBBRAlignmentCheckCoronal.png -a 0.8 -t -1x-1 -d 1 -p mask -s [5,mask,mask] -x ${tmpDir}/epi2highResBBREdgesRBGstep.nii.gz -f 0x1
