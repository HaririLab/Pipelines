...
for sub in $(ls ${rawDir}/DMHDS[01]*/anat/HighRes.nii | cut -d "/" -f10);do
	if [[ ! -f ${outDir}/${sub}/antCT/highRes_CorticalThicknessNormalizedToTemplate.nii.gz ]];then
		echo "mkdir -p /mnt/BIAC/munin2.dhe.duke.edu/Hariri/DBIS.01/Analysis/All_Imaging/${sub}/QA;/mnt/BIAC/munin2.dhe.duke.edu/Hariri/DNS.01/Analysis/Max/scripts/Pipelines/anat_DBIS.sh ${sub} 1 >> /mnt/BIAC/munin2.dhe.duke.edu/Hariri/DBIS.01/Analysis/All_Imaging/${sub}/QA/LOG.anat"
	fi
done
